...
class GetAutoCallPickupStateUseCase: SuspendingUseCase<Unit, AutoCallPickupDto>() {
    ...
    override suspend fun execute(params: Unit): ApiResult<AutoCallPickupDto> {
        return ApiResult.Success(AutoCallPickupDto(
            AutoCallPickupMode(systemManager.autoCallPickupState)
        ))
    }
}