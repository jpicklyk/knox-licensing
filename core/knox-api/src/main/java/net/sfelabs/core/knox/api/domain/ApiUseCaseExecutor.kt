...
suspend fun <T : Any> ApiUseCaseExecutor.executeWithTimeout(
    ...
    return execute {
        try {
            ...
            ApiResult.Error(DefaultApiError.TimeoutError(e.message.toString()), exception = e)
        }
    }
}
...
suspend fun <T : Any> ApiUseCaseExecutor.executeWithLogging(
    ...
    //android.util.Log.d(tag, "Executing use case")
    val result = execute(useCase)
    //android.util.Log.d(tag, "Use case result: $result")
    return result
}
